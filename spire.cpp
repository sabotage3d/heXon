    ...
    Enemy(context, masterControl, position)
{
    ...
    SubscribeToEvent(E_SCENEUPDATE, HANDLER(Spire, HandleSceneUpdate));
}

void Spire::HandleSceneUpdate(StringHash eventType, VariantMap &eventData)
{
    ...
    //Spin
    topNode_->Rotate(Quaternion(0.0f, timeStep*50.0f, 0.0f));
    bottomNode_->Rotate(Quaternion(0.0f, timeStep*-50.0f, 0.0f));
